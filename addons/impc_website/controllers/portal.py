...
class IMPCPortalController(http.Controller):
    ...
    def my_learning_dashboard(self, **kw):
        ...
        # Certificates
        certificates = request.env['impc.certificate'].sudo().search([
            ('partner_id', '=', partner.id),
            ('state', '=', 'issued'),
        ], order='completion_date desc', limit=5)
        ...
        for enrollment in in_progress:
            if enrollment.channel_id.learning_mode in ('hybrid', 'offline'):
                ...
                if event and event.date_begin and event.date_begin > fields.Datetime.now():
    ...
    def my_events(self, **kw):
        ...
        for enrollment in enrollments:
            ...
            if event:
                ...
                # Check attendance
                attendance = request.env['impc.session.attendance'].sudo().search([
                ...
                events_data.append({
                    ...
                    'attendance_status': attendance.attendance_status if attendance else 'pending',
                })
    ...
    def download_certificate(self, cert_id, **kw):
        ...
        return request.make_response(
            ...
            headers=[
                ...
                ('Content-Disposition', f'attachment; filename="{certificate.name}.pdf"'),
            ]
        )
